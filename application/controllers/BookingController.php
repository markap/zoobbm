...
class BookingController extends Zend_Controller_Action
{
	...
	/**
	 * ajax call for price calculating
	 */
	 ...
	 * showtickets
	 */
    public function showticketsAction()
    {
        $ticketDb = new Model_DbTable_Ticket();
		$this->view->tickets  = $ticketDb->getFutureTickets($this->userId);
		$this->view->oldTickets  = $ticketDb->getOldTickets($this->userId);
	}

	/**
	 * ajax storno of the tickets
	 */
    public function stornoajaxAction()
    {
        $this->_helper->layout->disableLayout();
		$ticketDb = new Model_DbTable_Ticket();
		$ticketDb->deleteTicket($this->_getParam('id'));
    }
